...
-- Create table for display settings if it doesn't exist
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'package_display_settings') THEN
    CREATE TABLE public.package_display_settings (
